...
namespace WindwoInstalTool
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            string dotNetPath = sysDisk + @"WINDOWS\Microsoft.NET\Framework\v4.0.30319\InstallUtil.exe"; //C:\Windows\Microsoft.NET\Framework\v4.0.30319\InstallUtil.exe
                //因为当前用的是4.0的环境  
            ...
            try
            {
                if (File.Exists(dotNetPath))
                {
                    ...
                    string ss = Cmd(cmd);
                    //LogerHelper.WriteOperateLog(ss);
